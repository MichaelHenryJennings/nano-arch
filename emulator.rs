...
fn main() -> Result<(), Error> {
    ...
    let mut mem : Vec<u32> = vec![0; 1 << 32];
    let mut k : usize = 0;
    while k < image.len() / 4 {
        mem[k] = (image[4 * k] as u32) + 
                 ((image[4 * k + 1] as u32) << 8) +
                 ((image[4 * k + 2] as u32) << 16) +
                 ((image[4 * k + 3] as u32) << 24);
        k += 1;
    }
